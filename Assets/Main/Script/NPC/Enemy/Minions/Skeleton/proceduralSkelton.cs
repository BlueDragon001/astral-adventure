...
public class proceduralSkelton : MonoBehaviour
{
    ...
    [SerializeField] private bool firstWillMove;
    private int _nLimbs;
    ...
    private void _HandleMovement()
    {
        ...
        if (limbToMove != -1)
        {
            ...
            if (limbToMove == 0) { firstWillMove = false; }
            if (limbToMove == 1) { firstWillMove = true; }
        }
    }

    void HandleSkeletolMovement()
    {
        ...
        if (_allLimbsResting)
        {
            _limbs[1].IKTarget.position = _limbs[1].lastPosition;
            Vector3 targetpoint = desiredPositions[0] + _velocity.normalized * _stepSize;
            targetpoint = _RaycastToGround(targetpoint, transform.up);
            targetpoint += transform.up * _feetOffset;
            _allLimbsResting = false;
            StartCoroutine(_Stepping(0, targetpoint));
